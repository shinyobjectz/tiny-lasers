...
- **Real production toolchains, not just language features.** The genuinely hard proof is that unmodified real-world libraries run byte-for-byte identical to Node, confined: the **rollup** bundler, the **Svelte 5** compiler, **Preact** and **SolidJS** server-side rendering, the **marked** Markdown parser, and the full **GSAP** animation engine (tweens, timelines, easing). These aren't test fixtures — they're the actual npm packages, compiled to the BEAM and producing the exact same bytes as `node`.
- **A slow request can't poison your fast ones.** This is the reason to run JavaScript on the BEAM. Node runs on a single event loop: one CPU-heavy request blocks *every* other request until it finishes. Measured head-to-head, same handler on both — when a genuinely-heavy (16.8 ms) request makes up just 5% of traffic, a Node server's fast-endpoint tail latency (p99) collapses to **~2,500 ms**; tiny-lasers stays at **~0.6 ms** (≈4,000×), because every request runs in its own isolated, preemptively-scheduled BEAM process. It stays flat as the load rises while Node's backlog only deepens. ([`bench/concurrency`](bench/concurrency))
- **Memory stays flat under sustained load, and a guest can't exhaust the host.** Serving thousands of requests through one runtime shows no memory growth — each request's state dies with its process. A guest that loops forever, allocates forever, or recurses forever is killed by a hard memory + wall-clock bound; every one of those attack vectors was tested, and the host survives untouched.
- **The execution model is production-shaped and load-proven.** A supervised, multi-tenant runtime — compile-once/run-per-request, per-tenant admission + rate limiting, per-invocation metering, and an atom-pressure watchdog that gracefully drains + recycles the node — sustained **~9,000 requests/second across eight tenants** in a load test, with output byte-correct, metering accurate to the invocation, memory flat, and a runaway guest contained through the full path.
...
## Part 8 — Where this wins, and where it doesn't

Honesty is part of the pitch, so here is the envelope, measured rather than asserted:

- **Node/V8 is faster per single request.** V8 is a world-class JIT; tiny-lasers compiles ahead-of-time to the BEAM without a tracing JIT, so on raw per-operation throughput it is roughly two orders of magnitude slower. If your workload is one fast request at a time and nothing else, there is no reason to leave Node.
- **tiny-lasers wins under concurrent load with mixed request costs.** The moment a genuinely-heavy request shares the runtime with fast ones — which every real server has (a report endpoint next to a health check, a big JSON parse next to a ping) — Node's single event loop makes the fast requests wait for the slow one, and tail latency collapses. tiny-lasers doesn't, because isolation and preemption are properties of the BEAM, not something you can bolt onto V8. This is a runtime-level advantage, not an optimization.
- **This is not a compute engine.** Heavy numeric work — matrix math, an ML model's tensor kernels — should not run in here; measured on a matmul, tiny-lasers is byte-for-byte correct but only ~0.1 MFLOP/s, because it's interpreter-class execution, not a JIT or a BLAS library. The right shape for that is to run the untrusted *orchestration* here (confined, concurrent) and hand the heavy math to a native library through a capability — the same way the sandbox already delegates layout, parsing, or the filesystem.

The one-line version: **Node is faster per request; tiny-lasers is faster under load, and it's safe by construction.** Real servers are governed by tail latency under load and by blast radius when something goes wrong — which is exactly where this wins.
