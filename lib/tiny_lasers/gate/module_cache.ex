defmodule TinyLasers.Gate.ModuleCache do
  ...
  **Per-tenant module cache for the warm-module / cold-process execution model.**
  ...
  def resolve(tenant, source, compile_fun, server \\ __MODULE__) when is_binary(source) and is_function(compile_fun, 1) do
    ...
    case :ets.lookup(@cache, key) do
      [{^key, module, src_len, _}] when src_len == byte_size(source) ->
        ...
        :ets.update_element(@cache, key, {4, mono()})
        {:ok, module}
  ...
  def checkout(module) do
    :ets.update_counter(@refs, module, {2, 1}, {module, 0})
    :ok
  end

  @doc "Release an in-flight reference."
  def checkin(module) do
    :ets.update_counter(@refs, module, {2, -1}, {module, 0})
    :ok
  end

  @doc "Run `fun` with `module` checked out for its whole lifetime (invocation bracket)."
  def with_module(module, fun) do
    checkout(module)
    try do
      fun.()
    after
  ...
  def init(opts) do
    ...
    :ets.new(@refs, [:named_table, :public, :set, write_concurrency: true])
      ...
      _ ->
        ...
        cond do
          MapSet.member?(tset, key) ->
            # key known to tenant but not in ETS (was evicted) — recompile into the SAME name (atom reused)
  ...
  defp do_compile(tenant, key, source, compile_fun, tset, state) do
    ...
    :ets.insert(@cache, {key, module, byte_size(source), mono()})
  ...
  defp maybe_evict(state) do
    if :ets.info(@cache, :size) <= state.max_entries do
      state
    else
      candidates =
        :ets.tab2list(@cache)
      ...
      Enum.find_value(candidates, state, fn {_used, k, mod} ->
        if in_flight(mod) == 0 and :code.soft_purge(mod) do
          :code.delete(mod)
          :ets.delete(@cache, k)
          :ets.delete(@refs, mod)
          state
  ...
  defp in_flight(mod) do
    case :ets.lookup(@refs, mod) do
      [{^mod, n}] -> n
      _ -> 0
    end
  end
