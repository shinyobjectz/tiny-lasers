(() => {
  ...
  var splitColor = function splitColor2(v, toHSL, forceAlpha) {
    var a2 = !v ? _colorLookup.black : _isNumber(v) ? [v >> 16, v >> 8 & _255, v & _255] : 0, r, g, b2, h, s, l, max, min, d, wasHSL;
    if (!a2) {
      if (v.substr(-1) === ",") {
    ...
    if (toHSL && !wasHSL) {
      ...
      if (max === min) {
        ...
        h = max === r ? (g - b2) / d + (g < b2 ? 6 : 0) : max === g ? (b2 - r) / d + 2 : (r - g) / d + 4;
        h *= 60;
      }
      a2[0] = ~~(h + 0.5);
      a2[1] = ~~(s * 100 + 0.5);
  ...
  (function(n, c2) {
    var n1 = 1 / c2, n2 = 2 * n1, n3 = 2.5 * n1, easeOut = function easeOut2(p) {
      return p < n1 ? n * p * p : p < n2 ? n * Math.pow(p - 1.5 / c2, 2) + 0.75 : p < n3 ? n * (p -= 2.25 / c2) * p + 0.9375 : n * Math.pow(p - 2.625 / c2, 2) + 0.984375;
    };
  ...
  var _initTween = function _initTween2(tween, time, tTime) {
    var vars = tween.vars, ease = vars.ease, startAt = vars.startAt, immediateRender = vars.immediateRender, lazy = vars.lazy, onUpdate = vars.onUpdate, runBackwards = vars.runBackwards, yoyoEase = vars.yoyoEase, keyframes = vars.keyframes, autoRevert = vars.autoRevert, dur = tween._dur, prevStartAt = tween._startAt, targets = tween._targets, parent = tween.parent, fullTargets = parent && parent.data === "nested" ? parent.vars.targets : targets, autoOverwrite = tween._overwrite === "auto" && !_suppressOverwrites, tl2 = tween.timeline, reverseEase = vars.easeReverse || yoyoEase, cleanVars, i, p, pt, target, hasPriority, gsData, harness, plugin, ptLookup, index, harnessVars, overwritten;
  ...
  var a = { x: 0, y: 0, opacity: 1, scale: 1 };
  var tw = gsapWithCSS.to(a, { x: 100, y: 50, opacity: 0, scale: 2, duration: 2, ease: "power2.inOut" });
  var samples = [];
  [0.5, 1, 1.5].forEach(function(t) {
    tw.seek(t);
    samples.push([fx(a.x), fx(a.y), fx(a.opacity), fx(a.scale)]);
  });
  var b = { p: 0 };
  var c = { q: 100 };
  var tl = gsapWithCSS.timeline();
  tl.to(b, { p: 200, duration: 1, ease: "back.out(1.7)" }).to(c, { q: 0, duration: 1, ease: "elastic.out(1, 0.3)" }, 0.5);
  tl.seek(0.75);
  var tlMid = [fx(b.p), fx(c.q)];
  tl.seek(2);
  var tlEnd = [fx(b.p), fx(c.q)];
  var items = [{ v: 0 }, { v: 0 }, { v: 0 }, { v: 0 }];
  var st = gsapWithCSS.to(items, { v: 10, duration: 0.5, stagger: 0.2, ease: "sine.inOut" });
  st.seek(0.6);
  var stagVals = items.map(function(o) {
    return fx(o.v);
  });
  print("GSAP_OK[" + JSON.stringify({ samples, tlMid, tlEnd, stagVals }) + "]");
})();
