(() => {
  ...
  var eases = ["power2.inOut", "power3.out", "back.out(1.7)", "elastic.out(1,0.3)", "sine.inOut", "expo.in", "circ.inOut", "bounce.out"];
  var rows = eases.map(function(name) {
    var e = gsapWithCSS.parseEase(name);
    return name + ":" + [0, 0.25, 0.5, 0.75, 1].map(function(p) {
      return fx(e(p));
    }).join(",");
  });
  print("EASE_OK[" + rows.join("|") + "]");
})();
